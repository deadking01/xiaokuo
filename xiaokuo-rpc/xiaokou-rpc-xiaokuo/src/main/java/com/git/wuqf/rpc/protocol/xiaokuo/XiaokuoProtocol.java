...
import com.git.wuqf.remoting.exchange.support.ExchangeHandlerAdapter;
...
public class XiaokuoProtocol extends AbstractProtocol {
    ...
    public static final String COMPATIBLE_CODEC_NAME = "xiaokuocompatible";
    ...
    private static XiaokuoProtocol INSTANCE;

    public XiaokuoProtocol() {
        INSTANCE = this;
    }

    public static XiaokuoProtocol getXiaokuoProtocol() {
        if (INSTANCE == null) {
            INSTANCE = new XiaokuoProtocol();
        }
        return INSTANCE;
    }
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException, RemotingException {
        ...
        XiaokuoExporter<T> exporter = new XiaokuoExporter<T>(invoker, key, exporterMap);
        ...
        if (isStubSupportEvent && !isCallbackservice) {
            ...
            if (stubServiceMethods == null || stubServiceMethods.length() == 0) {
                if (logger.isWarnEnabled()) {
                    logger.warn(String.valueOf(new IllegalStateException("consumer [" + url.getParameter(Constants.INTERFACE_KEY) +
...
//                logger.warn(new IllegalStateException("client is closed,but stay in clientmap .client :"+ client));
    ...
    private ExchangeClient initClient(URL url) throws RemotingException {
        ...
        url = url.addParameter(Constants.CODEC_KEY, Version.isCompatibleVersion() && compatible ? COMPATIBLE_CODEC_NAME : XiaokuoCodec.NAME);
        //默认开启heartbeat
        url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY, String.valueOf(Constants.DEFAULT_HEARTBEAT));
