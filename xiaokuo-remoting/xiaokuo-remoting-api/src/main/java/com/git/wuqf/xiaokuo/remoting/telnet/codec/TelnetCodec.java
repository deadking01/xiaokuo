 ...
 */
package com.git.wuqf.xiaokuo.remoting.telnet.codec;
...
import com.git.wuqf.xiaokuo.remoting.transport.codec.TransportCodec;
...
public class TelnetCodec extends TransportCodec {
    ...
    protected Object decode(Channel channel, ChannelBuffer buffer, int readable, byte[] message) throws IOException {
        ...
        for (Object command : EXIT) {
            if (isEquals(message, (byte[]) command)) {
                if (logger.isInfoEnabled()) {
                    logger.info(new Exception("Close channel " + channel + " on exit command: " + Arrays.toString((byte[]) command)).toString());
                }
                channel.close();
                return null;
            }
        ...
        for (Object command : EXIT) {
            if (isEquals(message, (byte[]) command)) {
                if (logger.isInfoEnabled()) {
                    logger.info(new Exception("Close channel " + channel + " on exit command " + command).toString());
                }
                channel.close();
                return null;
            }
